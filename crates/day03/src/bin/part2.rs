...
fn try_parse_num(yx: Coords, schematic: &Schematic) -> Option<Num> {
...
fn process(input: &str) -> Num {
    ...
    find_symbols(&schematic).into_iter()
        .filter_map(|(symbol, yx)| if symbol == '*' { Some(yx) } else { None })
        .filter_map(|symbol| {
            let adjs_coords = adjacents(symbol, (schematic.len() as _, schematic[0].len() as _));
            let adjs_nums = adjs_coords.into_iter().filter_map(|yx| try_parse_num(yx, &schematic)).collect::<HashSet<_>>();
            if adjs_nums.len() == 2 { Some(adjs_nums) } else { None }
        })
        .map(|adjs| adjs.into_iter().product::<Num>())
        .sum()
}
...
mod tests {
    #[test]
    fn example() {
        assert_eq!(crate::do_file("./example.txt"), 467835);
    }
}
